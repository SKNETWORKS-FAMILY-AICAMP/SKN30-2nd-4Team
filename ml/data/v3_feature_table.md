...
> ⚠️ **데이터 누수 방지 원칙**:
> `v2`의 실관람 요일 비율(`weekend_ratio`, `weekday_*_ratio`)은 **개봉 이후** 수집 데이터이므로 완전히 제외합니다.
> `v4` = `v1`(정적 메타) + `v3`(개봉 전 트렌드) — 누수 없는 최종 스펙입니다.

```
feature_table_v4.csv
─────────────────────────────────────────────────────────────────────────────
[식별]   movie_id, title
...
[트렌드] trend_pre7_avg, trend_pre7_max, trend_growth_rate,
         relative_search_share, has_trend_data
─────────────────────────────────────────────────────────────────────────────
총 32개 컬럼 (식별자 2개, 타겟 3개, 원본 피처 27개)
→ 전처리 후: 삭제 7개 + 신규 4개(is_new_*) = 최종 피처 24개
```
